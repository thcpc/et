...
from et_admin.user import service as user_service
from libs.http_handle import Success, JsonRequest
...
def login(request):
    ...
    if user_name:
        auth_token = user_service.login_by_name(user_name, password, finger_print)
    else:
...
def device(request):
    try:
        ...
        pay_load = user_service.device(_finger_print)
...
def register(request):
    ...
    user_name = r.get("username")
    password = r.get("password")
    resp_body = user_service.register(user_name, password)
...
def users(request):
    ...
    pay_load = user_service.users(self_user_id)
    return Success(payload=pay_load)