...
class ExecuteRegressionTaskGenerator(TaskGenerator):
    ...
    def match(self) -> bool:
        if self._work_on_doc.status >= Status.Document.Submit:
            for label in self._work_on_doc.labels:
                if label.detail.name == enums.Name.DocumentType.RegressionZh:
                    return True
        return False
    ...
    # @classmethod
    # def name(cls):
    #     return enums.Name.TaskType.RegressionTask
    ...
    #     _spec["spec"] = enums.Name.TaskType.RegressionTask
    #     return _spec
